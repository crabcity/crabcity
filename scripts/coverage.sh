...
if [[ $_skip_tests -eq 0 ]]; then
    if [[ -z "$_targets" ]]; then
        # Use cquery (not query) so target_compatible_with filters out
        # platform-incompatible targets (e.g. desktop app on Linux CI).
        _targets=$(bazel cquery 'kind("rust_test|jest_test", //...)' 2>/dev/null \
            | sed 's/ (.*//' | tr '\n' ' ')
        if [[ -z "$_targets" ]]; then
