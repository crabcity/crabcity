<script lang="ts">
	...
	function doColResize(e: PointerEvent, handleIndex: number) {
		...
		if (lw < MIN_COL_WIDTH) { rw += (lw - MIN_COL_WIDTH); lw = MIN_COL_WIDTH; }
		if (rw < MIN_COL_WIDTH) { lw += (rw - MIN_COL_WIDTH); rw = MIN_COL_WIDTH; }
		columnWidths[left] = lw;
		columnWidths[right] = rw;
	}

	function stopColResize() { draggingHandle = null; }
	...
	<div class="path-bar">
		...
		{:else}
			...
			<button class="edit-path-btn" onclick={startEditingPath} title="Type path directly">
				<svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
					<path d="M12.146.854a.5.5 0 0 1 .708 0l2.292 2.292a.5.5 0 0 1 0 .708l-9.5 9.5a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l9.5-9.5zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3-2.293-2.293L3 10.707V11h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l7.5-7.5z"/>
				</svg>
			</button>
		{/if}
	</div>
				...
				>
					...
					{:else}
						...
						<div class="new-dir-action">
							...
							{:else}
								<button class="new-dir-btn" onclick={() => startNewDir(colIndex)}>
									+ NEW FOLDER
								</button>
							{/if}
						</div>
		...
		<div class="git-panel">
			<div class="git-header">
				...
				<button class="git-info-btn" class:active={showGitDetail} onclick={toggleGitDetail} title="Repository details">
					<svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
						<path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 12.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zM8 6a.75.75 0 0 1 .75.75v4.5a.75.75 0 0 1-1.5 0v-4.5A.75.75 0 0 1 8 6zm0-2.5a.875.875 0 1 1 0 1.75.875.875 0 0 1 0-1.75z"/>
					</svg>
				</button>
						...
						{#if gitDetail.upstream}
							<div class="git-detail-row">
								...
								<span class="git-detail-value">
									...
									<span class="git-detail-counts">{@html `\u2191${gitDetail.upstream.ahead} \u2193${gitDetail.upstream.behind}`}</span>
								</span>
							</div>
						{/if}
						<div class="git-detail-row">
					...
					<div class="wt-form">
						<label class="wt-field">
							...
							<input
								...
								oninput={() => { worktreePath = suggestWorktreePath(worktreeBranch); }}
						...
						<div class="wt-actions">
							<button class="wt-cancel" onclick={() => { showWorktreeForm = false; }} disabled={creatingWorktree}>
								CANCEL
							</button>
							<button class="wt-create" onclick={handleCreateWorktree} disabled={!worktreeBranch || !worktreePath || creatingWorktree}>
								{creatingWorktree ? 'CREATING...' : 'CREATE'}
							</button>
						</div>
					</div>
				{:else}
					<button class="wt-add-btn" onclick={() => { showWorktreeForm = true; }}>
						+ NEW WORKTREE
					</button>
				{/if}
			</div>
...
<style>
	...
	.git-info-btn:hover, .git-info-btn.active { opacity: 1; color: var(--text-secondary); }
	...
	.wt-create:disabled, .wt-cancel:disabled {
		opacity: 0.5;
		cursor: not-allowed;
	}
</style>
