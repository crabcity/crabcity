<script lang="ts">
	...
	function handleOpenInSplit() {
		...
		splitPane(focusedId, 'vertical', defaultContentForKind('conversation', instance.id));
		onclose();
	}
...
<div class="instance-popover" style={popoverStyle}>
	...
	<!-- Open actions -->
	<button class="popover-item" onclick={handleOpenInFocused}>
		Open in focused pane
	</button>
	<button class="popover-item" onclick={handleOpenInSplit}>
		Open in new split
	</button>
