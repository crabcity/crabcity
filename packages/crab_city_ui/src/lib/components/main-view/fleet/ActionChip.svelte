<script lang="ts">
	...
	const turnInfo = $derived(
		item.event_type === 'completed_turn' && item.turn_count > 0
			? `${item.turn_count}`
			: null
	);
...
<style>
	...
	.critical .chip-verb { color: var(--status-red); }
	.error .chip-verb { color: var(--status-red); }
	.warning .chip-verb { color: var(--amber-400); }
	...
	@keyframes chip-pulse {
		0%, 100% { box-shadow: 0 0 6px color-mix(in srgb, var(--status-red) 20%, transparent); }
		50% { box-shadow: 0 0 10px color-mix(in srgb, var(--status-red) 35%, transparent); }
	}

	@media (prefers-reduced-motion: reduce) {
		.action-chip.critical { animation: none; }
	}
