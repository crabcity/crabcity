<script lang="ts">
	...
	const status = $derived(
		tool.is_error ? 'error' : tool.output ? 'complete' : 'running'
	);

	const agentType = $derived(
		typeof tool.input.subagent_type === 'string' ? tool.input.subagent_type : null
	);
	...
	const resultLong = $derived(
		(tool.output?.length ?? 0) > 500
	);
</script>

<div class="task-card" class:running={status === 'running'} class:complete={status === 'complete'} class:error={status === 'error'}>
	{#if showRaw}
		<div class="raw-view">
	...
	{:else}
		...
		<div class="card-header">
			<div class="header-left">
				...
				{/if}
				<span class="status-led" class:led-running={status === 'running'} class:led-complete={status === 'complete'} class:led-error={status === 'error'}></span>
			</div>
