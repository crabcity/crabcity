<script lang="ts">
	...
	$effect(() => {
		...
		apiGet<{ content: string }>(
			`/api/instances/${inst.id}/files/content?path=${encodeURIComponent(path)}`
		)
			.then((response) => {
...
<style>
	...
	.viewer-content::-webkit-scrollbar { width: 6px; height: 6px; }
	.viewer-content::-webkit-scrollbar-track { background: var(--surface-900); }
	.viewer-content::-webkit-scrollbar-thumb { background: var(--surface-400); border-radius: 3px; }
	.viewer-content::-webkit-scrollbar-thumb:hover { background: var(--amber-600); }
</style>
