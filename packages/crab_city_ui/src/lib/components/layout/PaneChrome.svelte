<script lang="ts">
	...
	import { instances, instanceList, currentInstanceId, selectInstance, createInstance, deleteInstance } from '$lib/stores/instances';
	...
	const SPLIT_KIND_OPTIONS: { kind: PaneContentKind; label: string; icon: string }[] = [
		{ kind: 'terminal', label: 'Terminal', icon: '<rect x="2" y="3" width="12" height="10" rx="1"/><polyline points="4 7 6 9 4 11"/>' },
		{ kind: 'conversation', label: 'Conversation', icon: '<path d="M2 4h12a1 1 0 0 1 1 1v6a1 1 0 0 1-1 1h-3l-2 2v-2H2a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1z"/>' },
		{ kind: 'file-explorer', label: 'Files', icon: '<path d="M1 4V3.5A.5.5 0 0 1 1.5 3H5l1.5 1.5H14.5a.5.5 0 0 1 .5.5v.5"/><rect x="1" y="4.5" width="14" height="9" rx=".5"/>' },
		{ kind: 'chat', label: 'Chat', icon: '<circle cx="8" cy="8" r="6"/><circle cx="5.5" cy="7.5" r=".7"/><circle cx="8" cy="7.5" r=".7"/><circle cx="10.5" cy="7.5" r=".7"/>' },
		{ kind: 'tasks', label: 'Tasks', icon: '<rect x="2" y="2" width="4" height="4" rx=".5"/><rect x="2" y="10" width="4" height="4" rx=".5"/><line x1="9" y1="4" x2="14" y2="4"/><line x1="9" y1="12" x2="14" y2="12"/>' },
		{ kind: 'file-viewer', label: 'File Viewer', icon: '<path d="M4 1h6l3 3v10a.5.5 0 0 1-.5.5H4a.5.5 0 0 1-.5-.5V1.5A.5.5 0 0 1 4 1z"/><polyline points="9.5 1 9.5 4.5 13 4.5"/>' },
		{ kind: 'git', label: 'Git', icon: '<circle cx="8" cy="3" r="1.5"/><circle cx="8" cy="13" r="1.5"/><circle cx="13" cy="8" r="1.5"/><line x1="8" y1="4.5" x2="8" y2="11.5"/><path d="M8 6.5c0 1.5 5 1.5 5 1.5"/>' },
		{ kind: 'settings', label: 'Settings', icon: '<circle cx="8" cy="8" r="2.5"/><path d="M8 1.5v2m0 9v2M3.4 3.4l1.4 1.4m6.4 6.4l1.4 1.4M1.5 8h2m9 0h2M3.4 12.6l1.4-1.4m6.4-6.4l1.4-1.4"/>' },
	];
...
{#if splitPopover}
	<div class="split-popover" style="left: {splitPopover.x}px; top: {splitPopover.y}px;">
		{#each SPLIT_KIND_OPTIONS as opt}
			<div
				class="split-popover-item"
				class:hovered={hoveredKind === opt.kind}
				data-split-kind={opt.kind}
			>
